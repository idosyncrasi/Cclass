...
point origin   = {0,  0}, errGen   = {-1, 0}, errOOB   = {-1, 1}, errNoDir = {-1, 2}, fin = {-1, -1};
...
void printHistPoint(struct pointId pnt, char* name){
    printf("%s%d.x: %d, %s%d.y: %d\n",
        name, pnt.id, pnt.pnt.y, name, pnt.id, pnt.pnt.x);
}
...
point ifDir(int maze[mw][mh], struct point pnt, bool debug){
